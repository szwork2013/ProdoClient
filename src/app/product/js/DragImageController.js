...
if (tests.dnd) { 
  ...
  { 
   ...
   if(e.dataTransfer.files[0].size/1024>500)
    { 
           $rootScope.ProdoAppMessage("Please drag image of size upto 500KB","error")  ; 
           $rootScope.count =0; 
    ...
    {
           $rootScope.ProdoAppMessage("Please verify extension of image dragged","error")  ;
           $rootScope.count =0; 
           e.dataTransfer.files = []; 
           $scope.validImageContent = 1;
    }
    ...
    { 
          $rootScope.ProdoAppMessage("You can only drag one image for a comment","error"); 
          $scope.validImageContent = 1; 
    }
